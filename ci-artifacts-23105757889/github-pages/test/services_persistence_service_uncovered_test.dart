...
class TestPersistenceService extends PersistenceService {
  ...
  Future<void> saveShooters(List<Shooter> shooters) async {
    store['shooters'] = shooters;
  }

  Future<void> saveStages(List<MatchStage> stages) async {
    store['stages'] = stages;
  }
...
void main() {
  ...
  group('PersistenceService uncovered branches', () {
    ...
    test(
      'future-proof: clears data if schema version is downgraded (mocked, injectable)',
      () async {
        ...
        SharedPreferences.setMockInitialValues({
          ...
          'shooters': '[{"name":"A","scaleFactor":1.0}]',
        });
        ...
        expect(prefs.getInt('dataSchemaVersion'), kDataSchemaVersion);
        expect(prefs.getString('shooters'), isNull);
      },
    );
    test(
      'ensureSchemaUpToDate sets version if missing (mocked, injectable)',
      () async {
        // This test injects a mock SharedPreferences instance into PersistenceService.
    ...
    test('calls _migrateSchema when upgrading schema', () async {
      ...
      final prefs = await SharedPreferences.getInstance();
      var called = false;
      final service = _TestMigrationPersistenceService(
        prefs: prefs,
        onMigrate: () => called = true,
      );
      await service.ensureSchemaUpToDate();
...
class _TestMigrationPersistenceService extends PersistenceService {
  ...
  _TestMigrationPersistenceService({
